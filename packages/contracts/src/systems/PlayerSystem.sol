...
import { Position, PositionTableId, PositionData, Health, HealthData, Strength } from "../codegen/Tables.sol";
...
contract PlayerSystem is System {
  function spawn(int32 x, int32 y) public {
    ...
    Health.set(player, HealthData({ max: 100, current: 100 }));
    Strength.set(player, 25);
  }

  function move(Direction direction) public {
    require(IWorld(_world()).stakingHasUnlockableShare(_msgSender()), "insufficient staking share");

    require(direction != Direction.Unknown, "invalid direction");

    bytes32 player = addressToEntity(_msgSender());
    int32 existingStrength = Strength.get(player);
    require(existingStrength != 0, "player not spawned");
    ...
    IWorld(_world())._privateDeFiIncreaseStakingShareUnlockedUpTo(_msgSender(), 200);
  }
}
